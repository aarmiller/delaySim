...
compute_miss_summary <- function(sim_miss_num_data){
  ...
  out <- sim_miss_num_data %>%
    dplyr::summarise(n_pat = dplyr::n(),                          # number of patients missed
                     n_pat_inpatient = dplyr::sum(n_vis_inpatient>0), # number of patients that had inpatient miss
                     n_pat_outpatient = dplyr::sum(n_vis_out>0),  # number of patients that had outpatient miss
                     n_pat_ed = dplyr::sum(n_vis_ed>0),      # number of patients that had inpatient miss
